...
export function EventCard({ event, showOrganizer = true, variant = 'default' }: EventCardProps) {
  ...
  return (
    <article className="e-card gcard rounded-2xl overflow-hidden flex flex-col h-full">
      ...
      {/* ── Image area ── rich gradient, no placeholder text visible ── */}
      <Link href={`/events/${event.id}`} className="block relative flex-shrink-0">
        <div className={`${IMG_CLASS[event.category]} ${imgH} w-full`}>
          ...
          <span className={`absolute bottom-3 left-3 tag ${cat.tagClass} z-10`}>{cat.emoji} {cat.label}</span>
          ...
          {/* Photo note (dev reference — invisible to end user) */}
      ...
      <div className="p-5 flex flex-col flex-1">
        ...
        {/* ── Bottom row: organizer + price/bookmark ── */}
        <div className="flex items-center justify-between pt-3.5 border-t border-white/[0.06]">

          {/* Organizer — avatar + name + verified badge (Twitter/Instagram pattern) */}
          {showOrganizer && (
            <Link
            ...
            >
              {/* Avatar */}
              <div className={`w-6 h-6 rounded-full ${event.orgColor} flex items-center justify-center flex-shrink-0 text-[0.5rem] font-display font-bold text-lavender border border-white/10`}>
                {event.orgInitials.charAt(0)}
              </div>

              {/* Name + verified badge inline — same as Twitter */}
          ...
          <div className="flex items-center gap-2.5 flex-shrink-0 ml-auto">
            ...
            <span className={`font-display font-semibold text-sm ${isFree ? 'text-emerald-400' : 'text-lavender'}`}
              style={{ color: isFree ? '#34d399' : '#C4B5FD' }}>
