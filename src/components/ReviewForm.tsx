...
export function ReviewForm({ eventId }: Props) {
  ...
  async function handleSubmit() {
    ...
    if (incomplete) { setError('Please rate all four categories before submitting.'); return }
  ...
  return (
    <div className="gcard rounded-2xl p-6 mt-6">
      ...
      {stats && (
        <div className="mb-6">
          <div className="flex items-center gap-3 mb-3">
            <span className="font-display font-bold text-3xl" style={{ color: '#C2820D' }}>
              ...
              <div className="flex gap-0.5 mb-0.5">
                {[1,2,3,4,5].map(n => (
                  <svg key={n} width="12" height="12" viewBox="0 0 24 24"
                    fill={n <= Math.round(stats.overallAvg) ? '#C2820D' : 'none'}
                    stroke="#C2820D" strokeWidth="2">
                    <polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/>
                  </svg>
                ))}
              </div>
              <p className="font-body text-xs" style={{ color: 'rgba(240,232,214,0.38)' }}>
          ...
          <div className="px-4 py-3 rounded-xl mb-4"
            style={{ background: 'rgba(31,122,104,0.12)', border: '1px solid rgba(31,122,104,0.20)' }}>
            <p className="font-body text-sm font-semibold" style={{ color: '#27967F' }}>
          ...
          <div className="space-y-2.5">
            {[
              ...
              <div key={label} className="flex items-center gap-3">
                <span className="font-body text-xs w-20 flex-shrink-0"
                  style={{ color: 'rgba(240,232,214,0.44)' }}>{label}</span>
                ...
                <span className="font-body text-xs w-6 text-right flex-shrink-0"
                  style={{ color: '#C2820D' }}>{val.toFixed(1)}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* Review form */}
      {!userId ? (
        <p className="font-body text-sm" style={{ color: 'rgba(240,232,214,0.34)' }}>
          <a href="/login" style={{ color: '#C2820D' }}>Log in</a> to leave a review.
        </p>
      ) : !attended ? (
        <p className="font-body text-sm" style={{ color: 'rgba(240,232,214,0.34)' }}>
          Only verified attendees can submit reviews. Mark your attendance on the event page.
        </p>
      ) : reviewed || submitted ? (
        <div className="flex items-center gap-2 p-4 rounded-xl"
          style={{ background: 'rgba(31,122,104,0.12)', border: '1px solid rgba(31,122,104,0.20)' }}>
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#27967F" strokeWidth="2">
            <path d="M20 6L9 17l-5-5"/>
          </svg>
          <span className="font-body text-sm font-semibold" style={{ color: '#27967F' }}>
            {submitted ? 'Review submitted — thank you.' : 'You have already reviewed this event.'}
          </span>
        </div>
      ) : (
        <div>
          <p className="font-body text-sm mb-5" style={{ color: 'rgba(240,232,214,0.44)' }}>
            ...
            {METRICS.map(({ key, label, sub }) => (
              <div key={key}>
                <div className="flex items-center justify-between mb-1">
                  <span className="font-body text-sm font-semibold" style={{ color: '#F0E8D6' }}>{label}</span>
                  <span className="font-body text-xs" style={{ color: 'rgba(240,232,214,0.34)' }}>{sub}</span>
                </div>
                <div className="flex gap-2">
                  {[1,2,3,4,5].map(n => (
                    <button key={n} type="button" onClick={() => setScores(s => ({ ...s, [key]: n }))}
                      ...
                      style={{
                        background: scores[key] >= n ? 'rgba(194,130,13,0.20)' : 'rgba(240,232,214,0.05)',
                        color:      scores[key] >= n ? '#C2820D'               : 'rgba(240,232,214,0.30)',
                        border:     `1px solid ${scores[key] >= n ? 'rgba(194,130,13,0.35)' : 'rgba(240,232,214,0.08)'}`,
                      }}>
                      {n}
                    </button>
                  ))}
          ...
          {error && (
            <p className="text-xs font-body text-red-400 mb-4">{error}</p>
          )}
