      ...
      <div className="page-header">
        ...
        <div className="container-page relative z-10">
          ...
          <p className="sr font-body text-base max-w-lg" style={{ color: 'rgba(240,234,255,0.38)', fontSize: '1rem' }}>
            Curated events across your Supabase-backed ecosystem. Add records in the database and they appear here automatically.
          </p>
        </div>
      </div>
      ...
      <div className="container-page py-10 pb-24">
        <div className="lg:hidden mb-5 flex gap-3">
          <div className="relative flex-1">
            <svg className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2" style={{ color: 'rgba(240,234,255,0.22)' }}>
          ...
          <button onClick={() => setSidebarOpen((o) => !o)} className="btn-ghost px-4 py-2 text-sm flex-shrink-0">Filter</button>
        </div>
        ...
        <div className="grid lg:grid-cols-[280px_1fr] gap-8">
          <aside className={`${sidebarOpen ? 'block' : 'hidden'} lg:block`}>
            <div className="gcard rounded-2xl p-5 sticky top-24 space-y-6">
              <div>
                ...
                <input className="form-input" type="text" placeholder="Search by title or organizer" value={search} onChange={(e) => setSearch(e.target.value)} />
              </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {CATEGORY_ORDER.map((cat) => (
                    <button
                      ...
                      className={`tag ${CATEGORY_META[cat].tagClass} ${activeCats.includes(cat) ? 'ring-1 ring-[#7B3FE4]' : ''}`}
                      type="button"
                    >
                ...
                <div className="space-y-2">
                  {FORMATS.map((format) => (
                    <button
                      key={format.key}
                      type="button"
                      onClick={() => setActiveFormat((current) => current === format.key ? null : format.key)}
                      className="w-full text-left px-4 py-3 rounded-2xl text-sm font-body transition-all"
                      style={{
                        background: activeFormat === format.key ? 'rgba(123,63,228,0.14)' : 'rgba(255,255,255,0.03)',
                        color: activeFormat === format.key ? '#F0EAFF' : 'rgba(240,234,255,0.36)',
              ...
              <label className="flex items-center gap-3 px-4 py-3 rounded-2xl" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(196,181,253,0.12)' }}>
                <input type="checkbox" checked={freeOnly} onChange={(e) => setFreeOnly(e.target.checked)} />
                <span className="text-sm font-body" style={{ color: '#F0EAFF' }}>Free events only</span>
              </label>
            </div>
          </aside>
          ...
          <main>
            <div className="flex items-center justify-between mb-5">
              <p className="text-sm font-body" style={{ color: 'rgba(240,234,255,0.34)' }}>
            ...
            {loading ? (
              ...
              <div className="gcard rounded-2xl p-12 text-center">
                ...
                <h3 className="font-display font-semibold mb-2" style={{ color: '#F0EAFF' }}>No matching events</h3>
                <p className="font-body text-sm" style={{ color: 'rgba(240,234,255,0.34)' }}>
                  Add more events in Supabase or adjust your filters.
                </p>
              </div>
            ) : (
