...
export default function OrganizerLayout({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="min-h-screen flex" style={{ background: '#0D0719' }}>
      ...
      <aside className={`dash-sidebar fixed lg:static z-40 transition-transform duration-300 ${mobile ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'} top-0 lg:top-0 pt-20 lg:pt-6`}>
        <Link href="/" className="hidden lg:flex items-center gap-2 px-3 mb-8">
          ...
          <span className="font-display font-bold text-sm" style={{ color: '#F0EAFF' }}>
            Verivent<span style={{ color: '#7B3FE4' }}>.</span>
          </span>
        </Link>
        ...
        <div className="px-3 mb-6 pb-5" style={{ borderBottom: '1px solid rgba(196,181,253,0.07)' }}>
          <div className="flex items-center gap-3 mb-2">
            <div className="w-9 h-9 rounded-full flex items-center justify-center font-display font-bold text-sm flex-shrink-0" style={{ background: 'rgba(123,63,228,0.25)', color: '#C4B5FD' }}>
              {initials}
            </div>
          ...
          {!isVerified && (
            <Link href="/organizers/apply" className="flex items-center gap-2 px-3 py-2 rounded-xl text-xs font-body mt-2 transition-all" style={{ background: 'rgba(123,63,228,0.1)', border: '1px solid rgba(123,63,228,0.25)', color: '#C4B5FD' }}>
