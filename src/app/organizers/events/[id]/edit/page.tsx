...
export default function EditEventPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const event = await getOrganizerEventById(eventId, auth.user.id)
      if (!mounted) return
      ...
      setLongDesc(event.longDesc || event.description)
      setCategory(event.category)
      setFormat(event.format)
      setDate(event.dateRaw?.slice(0, 10) || '')
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    setSaving(true)
    try {
      await updateOrganizerEvent(eventId, userId, {
        title: title.trim(),
      ...
      console.error('updateOrganizerEvent failed', err)
      setError('We could not save your changes. Please check your details and try again.')
      setSaving(false)
    }
  }
      ...
      <div className="p-6 md:p-10 max-w-3xl">
        <div className="gcard rounded-2xl p-10 text-center">
          ...
          <p className="font-body text-sm mb-6" style={{ color: 'var(--v-muted)' }}>
            This event doesn't exist, or it doesn't belong to your organizer account.
          </p>
          <Link href="/organizers/events" className="btn-pri px-7 py-3 text-sm">Back to My Events</Link>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="p-6 md:p-10 max-w-3xl">
      ...
      <h1 className="h-lg mb-1" style={{ color: 'var(--v-text)' }}>Edit Event</h1>
