...
export default function OrganizerProfilePage({ params }: { params: { id: string } }) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-8">
        <div className="gcard rounded-2xl p-8 text-center max-w-md">
          <h1 className="h-md mb-2" style={{ color: '#F0E8D6' }}>Organizer not found</h1>
          <p className="font-body text-sm" style={{ color: 'rgba(240,232,214,0.35)' }}>
            The organizer profile is not available yet.
          </p>
          <Link href="/organizers" className="btn-pri inline-flex px-6 py-3 text-sm mt-5">
            Back to Organizers
          </Link>
        </div>
      </div>
  ...
  const topCategories = Array.from(new Set(events.map(e => e.category))).slice(0, 5)

  return (
    <div className="min-h-screen">

      {/* Cover */}
      <div className="w-full relative overflow-hidden mt-[66px]"
        style={{ height: '260px', background: organizer.coverColor }}>
        {organizer.coverUrl && (
          <img src={organizer.coverUrl} alt={`${organizer.name} cover`}
            className="absolute inset-0 w-full h-full object-cover" />
        )}
        <div className="absolute inset-0"
          style={{ background: 'linear-gradient(to top, rgba(6,9,26,0.7) 0%, transparent 60%)' }} />
      </div>
      ...
      <div className="container-page">
        ...
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-5 -mt-10 mb-10 relative z-10">
          <div className="flex items-end gap-5">
            <div
              className="w-24 h-24 rounded-3xl flex items-center justify-center font-display font-bold text-2xl flex-shrink-0"
              style={{
                background: organizer.avatarColor,
                color:      '#D4970F',
                border:     '1px solid rgba(194,130,13,0.16)',
                boxShadow:  '0 0 0 4px #06091A',
              }}>
              {organizer.avatarUrl ? (
                <img src={organizer.avatarUrl} alt={organizer.name}
            ...
            <div className="pb-1">
              <div className="flex flex-wrap items-center gap-2.5 mb-1">
                <h1 className="font-display font-bold text-3xl"
                  style={{ color: '#F0E8D6', letterSpacing: '-0.02em' }}>
                  {organizer.name}
                </h1>
          ...
          <div className="flex items-center gap-3 pb-1">
            ...
            <Link href="/organizer/apply" className="btn-pri px-6 py-2.5 text-sm">
              Apply as Organizer
            </Link>
          </div>
        </div>
          ...
          {[
            ...
            <div key={l} className="gcard rounded-2xl p-5 text-center">
              <div className="font-display font-bold mb-1"
                style={{ color: '#F0E8D6', fontSize: '1.5rem' }}>{v}</div>
              <div className="text-xs font-body"
                style={{ color: 'rgba(240,232,214,0.28)' }}>{l}</div>
            </div>
          ))}
        </div>
        ...
        <div className="grid lg:grid-cols-[280px_1fr] gap-12 pb-24">
          ...
          <div className="sr space-y-6">
            <div>
              <h2 className="h-md mb-4" style={{ color: '#F0E8D6' }}>About</h2>
              <p className="font-body text-sm leading-relaxed mb-5"
                style={{ color: 'rgba(240,232,214,0.38)' }}>{organizer.longBio}</p>

              <div className="flex flex-wrap gap-2 mb-6">
                {(organizer.categories || []).map(cat => (
                  <span key={cat} className={`tag ${CATEGORY_META[cat].tagClass}`}>
              ...
              <div className="space-y-3">
                {[
                  ...
                  <a key={item.icon + item.label} href={item.href || '#'}
                    target="_blank" rel="noreferrer"
                    className="flex items-center gap-3 transition-colors"
                    style={{ color: 'rgba(240,232,214,0.32)' }}>
                    <span style={{ color: '#C2820D' }}>{item.icon}</span>
            ...
            <div className="gcard rounded-2xl p-5"
              style={{ border: '1px solid rgba(194,130,13,0.16)' }}>
              <p className="text-xs font-body leading-relaxed mb-4"
                style={{ color: 'rgba(240,232,214,0.36)' }}>
                Want to become a verified organizer on Verive?
              </p>
              <Link href="/organizer/apply"
                className="btn-pri text-xs px-5 py-2.5 w-full justify-center">
                Apply Now
              </Link>
            </div>

            {/* ── Reputation metrics — algorithm output ── */}
            <OrganizerMetrics organizerId={params.id} />
          </div>
          ...
          <div>
            <div className="sr mb-12">
              <div className="flex items-center justify-between mb-6">
                <h2 className="h-md" style={{ color: '#F0E8D6' }}>Upcoming Events</h2>
                <span className="text-xs font-body"
                  style={{ color: 'rgba(240,232,214,0.26)' }}>{events.length} listed</span>
              </div>
              {events.length > 0 ? (
                ...
                <div className="gcard rounded-2xl p-8 text-center">
                  <p className="text-sm font-body" style={{ color: 'rgba(240,232,214,0.28)' }}>
            ...
            <div className="sr">
              <h2 className="h-md mb-6" style={{ color: '#F0E8D6' }}>Categories</h2>
