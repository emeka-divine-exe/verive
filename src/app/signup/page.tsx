...
export default function SignupPage() {
  ...
  if (done) {
    return (
      <div className="min-h-screen flex items-center justify-center relative overflow-hidden grid-bg px-4">
        <div className="orb w-96 h-96 left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2" style={{ background: 'rgba(123,63,228,0.15)' }} />
        <div className="auth-card relative z-10 text-center">
          ...
          <h2 className="h-lg mb-3" style={{ color: '#F0EAFF' }}>Check your inbox</h2>
          <p className="font-body text-sm leading-relaxed mb-6" style={{ color: 'rgba(240,234,255,0.42)' }}>
            We sent a verification link to <strong style={{ color: '#C4B5FD' }}>{email}</strong>. Click it to activate your account.
          </p>
  ...
  return (
    <div className="min-h-screen flex items-center justify-center relative overflow-hidden grid-bg px-4 py-24">
      <div className="orb w-96 h-96 -right-32 top-0" style={{ background: 'rgba(123,63,228,0.18)' }} />
      <div className="orb w-80 h-80 left-0 bottom-0 -translate-x-1/3" style={{ background: 'rgba(196,181,253,0.06)' }} />
      ...
      <div className="relative z-10 w-full flex flex-col items-center">
        ...
        <div className="auth-card">
          <h1 className="h-lg text-center mb-2" style={{ color: '#F0EAFF' }}>Create an account</h1>
          <p className="text-center font-body text-sm mb-6" style={{ color: 'rgba(240,234,255,0.38)' }}>
            Join Verivent and start discovering verified events
          </p>
          ...
          <div className="grid grid-cols-2 gap-3 mb-6">
            {([
              ...
              <button key={key} type="button" onClick={() => setRole(key)}
                ...
                style={{
                  background: role === key ? 'rgba(123,63,228,0.18)' : 'rgba(26,16,50,0.6)',
                  border: `1px solid ${role === key ? 'rgba(123,63,228,0.5)' : 'rgba(196,181,253,0.1)'}`,
                }}>
                <div className="text-xl mb-2">{icon}</div>
                <div className="font-display font-semibold text-xs mb-0.5" style={{ color: '#F0EAFF' }}>{label}</div>
                <div className="text-[0.65rem] font-body" style={{ color: 'rgba(240,234,255,0.38)' }}>{sub}</div>
              </button>
            ))}
          </div>
          ...
          {role === 'organizer' && (
            <p className="text-[0.68rem] font-body text-center mt-4 leading-relaxed" style={{ color: 'rgba(240,234,255,0.28)' }}>
              Organizer accounts are created instantly. Verified badge is granted manually after review.
            </p>
          )}

          <p className="text-center text-sm font-body mt-5" style={{ color: 'rgba(240,234,255,0.35)' }}>
            Already have an account?{' '}
            <Link href="/login" className="font-semibold" style={{ color: '#C4B5FD' }}>Log in</Link>
          </p>
        </div>
