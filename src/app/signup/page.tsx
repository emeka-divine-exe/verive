      ...
      <div className="min-h-screen flex items-center justify-center relative overflow-hidden grid-bg px-4">
        ...
        <div className="auth-card relative z-10 text-center">
          <div className="w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-6"
            style={{ background: 'rgba(194,130,13,0.14)', border: '1px solid rgba(194,130,13,0.25)' }}>
            <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#C2820D" strokeWidth="2">
              <path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/>
              <polyline points="22,6 12,13 2,6"/>
            </svg>
          </div>
          <h2 className="h-lg mb-3" style={{ color: '#F0E8D6' }}>Check your inbox</h2>
          <p className="font-body text-sm leading-relaxed mb-6" style={{ color: 'rgba(240,232,214,0.44)' }}>
            We sent a verification link to{' '}
            <strong style={{ color: '#C2820D' }}>{email}</strong>.{' '}
    ...
    <div className="min-h-screen flex items-center justify-center relative overflow-hidden grid-bg px-4 py-24">
      ...
      <div className="relative z-10 w-full flex flex-col items-center">
        ...
        <div className="auth-card">
          <h1 className="h-lg text-center mb-2" style={{ color: '#F0E8D6' }}>Create an account</h1>
          <p className="text-center font-body text-sm mb-6" style={{ color: 'rgba(240,232,214,0.40)' }}>
          ...
          <div className="grid grid-cols-2 gap-3 mb-6">
            {([
              ...
              <button key={key} type="button" onClick={() => setRole(key)}
                ...
                style={{
                  background: role === key ? 'rgba(194,130,13,0.14)' : 'rgba(13,18,48,0.6)',
                  border: `1px solid ${role === key ? 'rgba(194,130,13,0.40)' : 'rgba(240,232,214,0.08)'}`,
                }}>
                <div className="text-xl mb-2">{icon}</div>
                <div className="font-display font-semibold text-xs mb-0.5" style={{ color: '#F0E8D6' }}>{label}</div>
                <div className="text-[0.65rem] font-body" style={{ color: 'rgba(240,232,214,0.38)' }}>{sub}</div>
              </button>
            ))}
          </div>
          ...
          <button onClick={handleGoogle} disabled={googleLoad}
            ...
            style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(240,232,214,0.10)', color: '#F0E8D6' }}>
            {googleLoad
          ...
          {role === 'organizer' && (
            <p className="text-[0.68rem] font-body text-center mt-4 leading-relaxed"
              style={{ color: 'rgba(240,232,214,0.28)' }}>
              Organizer accounts are created instantly. Verified badge is earned through sustained event quality.
            </p>
          )}

          <p className="text-center text-sm font-body mt-5" style={{ color: 'rgba(240,232,214,0.36)' }}>
            Already have an account?{' '}
            <Link href="/login" className="font-semibold" style={{ color: '#C2820D' }}>Log in</Link>
          </p>
        </div>

        <div className="flex items-center gap-2 mt-6">
          <VerifiedBadge size={13} />
          <span className="text-xs font-body" style={{ color: 'rgba(240,232,214,0.22)' }}>
